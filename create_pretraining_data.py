...
class TrainingInstance(object):
    ...
    def __init__(self, tokens_mlm_nsp, segment_ids_mlm_nsp, speaker_ids_mlm_nsp, is_random_next, masked_lm_positions,
                 ...
                 pointer_cd_positions_adr2, pointer_cd_positions_spk3, pointer_cd_positions_adr3,
...
def create_training_instances(ctx_list, ctx_spk_list, ctx_adr_list, ctx_relation_list, rsp_list, rsp_spk_list,
                              rsp_adr_list, rsp_relation_list,
                              ...
                              max_predictions_per_seq_cd, rng):
    ...
    for _ in tqdm(range(dupe_factor)):
        for i in tqdm(range(len(ctx_list))):
            ...
            instance = create_instances_from_document(
                ctx_tokens, ctx_spk, ctx_adr, ctx_relation, rsp_pos_tokens, rsp_neg_tokens, rsp_spk, rsp_adr,
                rsp_relation,
                thread_a_tokens, thread_a_spk, thread_b_tokens, thread_b_spk, is_random_next_snd,
                max_seq_length, max_utr_length, masked_lm_prob,
                max_predictions_per_seq, max_predictions_per_seq_ar, max_predictions_per_seq_sr,
                max_predictions_per_seq_cd, vocab_words, rng)
...
def create_instances_from_document(
        ...
        thread_a_tokens, thread_a_spk, thread_b_tokens, thread_b_spk, is_random_next_snd,
        max_seq_length, max_utr_length, masked_lm_prob,
        max_predictions_per_seq, max_predictions_per_seq_ar, max_predictions_per_seq_sr, max_predictions_per_seq_cd,
        vocab_words, rng):
...
def create_identical_speaker_searching_predictions(speaker_ids, ctx_spk, cls_positions, max_predictions_per_seq_sr,
                                                   rng):
...
def create_masked_shared_utterance_restoration_predictions(tokens, ctx_relation, cls_positions, max_utr_length,
                                                           vocab_words, rng):
...
def create_shared_node_detection_predictions(thread_a_tokens, thread_a_spk, thread_b_tokens, thread_b_spk,
                                             max_seq_length, rng):
...
def main(_):
  ...
  """
    for train_file in [FLAGS.train_file]:
        ...
        tf.compat.v1.logging.info(
            "Reading from input file {} --> {} context-response pairs".format(train_file, len(ctx_list)))

    # 2. create training instances
    rng = random.Random(FLAGS.random_seed)
    instances = create_training_instances(
        ctx_list, ctx_spk_list, ctx_adr_list, ctx_relation_list, rsp_list, rsp_spk_list, rsp_adr_list,
        rsp_relation_list,
        tokenizer, FLAGS.max_seq_length, FLAGS.max_utr_length, FLAGS.dupe_factor, FLAGS.masked_lm_prob,
        FLAGS.max_predictions_per_seq, FLAGS.max_predictions_per_seq_ar, FLAGS.max_predictions_per_seq_sr,
        FLAGS.max_predictions_per_seq_cd, rng)
    ...
    write_instance_to_example_files(
        ...
        FLAGS.max_predictions_per_seq, FLAGS.max_predictions_per_seq_ar, FLAGS.max_predictions_per_seq_sr,
        FLAGS.max_predictions_per_seq_cd, output_files)
